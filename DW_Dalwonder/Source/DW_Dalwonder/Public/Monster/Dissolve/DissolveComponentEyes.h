...
#include "DissolveComponentEyes.generated.h"
...
struct FMeshMaterialKey
{
	...
	FMeshMaterialKey() : Mesh(nullptr), MaterialIndex(0) {}
	FMeshMaterialKey(UMeshComponent* InMesh, int32 InIndex)
		: Mesh(InMesh), MaterialIndex(InIndex) {
	}
...
FORCEINLINE uint32 GetTypeHash(const FMeshMaterialKey& Key)
{
	return HashCombine(::GetTypeHash(Key.Mesh), ::GetTypeHash(Key.MaterialIndex));
}
...
class DW_DALWONDER_API UDissolveComponentEyes : public UActorComponent
{
	GENERATED_BODY()

public:
	UDissolveComponentEyes();
...
protected:
	...
	void FindAllMeshComponentsRecursive(USceneComponent* Root);
...
private:
	...
	UPROPERTY()
	TMap<FMeshMaterialKey, UMaterialInstanceDynamic*> DynamicMaterialMap;
